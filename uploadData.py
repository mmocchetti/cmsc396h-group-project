...
def create_databse():
    ...
    cmd = 'CREATE TABLE IF NOT EXISTS weather (state VARCHAR(50), myDate DATE, temp INT, frshtt VARCHAR(6))'
    cursor.execute(cmd)
    ...
    with open(filepath) as fp:  
        ...
        while line:
            if counter == 1:
                ...
                frshtt      = str(split_string[21]).strip()
                
                insertStatement = "INSERT INTO weather (state, myDate, temp, frshtt) VALUES ('" + state + "','" + date_string + "'," + temp_string + ",'" + frshtt + "')"
